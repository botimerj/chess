            ...
            for(int i = 0; i < 8; i++){ for(int j = 0; j < 8; j++){
                if(game->ts_color(game->bstate[i][j]) == game->turn){
                    ...
                    for(std::list<glm::ivec2>::iterator itt = move_list.begin(); itt != move_list.end(); ++itt){
                        ...
                        // Go deeper
                        int opposing_max_total = -100000;
                        for(int k = 0; k < 8; k++){ for(int l = 0; l < 8; l++){
                            if(game->ts_color(game->bstate[k][l]) == !game->turn){
                                move_list1 = game->valid_moves(game->bstate[k][l], glm::ivec2(k,l));
                                for(std::list<glm::ivec2>::iterator itt1 = move_list1.begin(); itt1 != move_list1.end(); ++itt1){
                                    // Save old state
                                    sel1 = game->bstate[k][l];
                                    glm::ivec2 idx1 = *itt1;
                                    TS bstate_old1 = game->bstate[idx1.x][idx1.y];

                                    game->bstate[k][l] = TS::e;
                                    game->bstate[idx1.x][idx1.y] = sel1;

                                    // Evaluate
                                    int good_total = 0;
                                    int bad_total = 0;
                                    for(int m = 0; m < 8; m++){ for(int n = 0; n < 8; n++){
                                        int mod = piece_value(game->bstate[m][n]);
                                        if(game->ts_color(game->bstate[m][n]) ==  !game->turn) good_total += mod;
                                        else if( game->ts_color(game->bstate[m][n]) == game->turn ) bad_total  += mod;
                                        int total = good_total - bad_total;
                                        if(total-opposing_max_total>0){
                                            //from = glm::ivec2(k,l);
                                            //to = idx;
                                            opposing_max_total = total;
                                        }
                                    }}
                                    // Reset to old state
                                    game->bstate[k][l] = sel1;
                                    game->bstate[idx1.x][idx1.y] = bstate_old1;
                                }
                            }
                        }}

                        //if(opposing_max_total < min_max || rand()%10 > 8){
                        if(opposing_max_total < min_max || rand()%100 > 99){
                            from = glm::ivec2(i,j);
                            to = idx;
                            min_max = opposing_max_total;
                        }
